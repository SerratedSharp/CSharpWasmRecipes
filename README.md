...
# C# WASM Recipes
A collection of code snippets, utilities, and guidance for C# WASM, primarily focused on browser hosted WASM using Uno.Wasm.Bootstrap.  The focus being on non-Blazor and non-UnoPlatform applications, as there is already a wealth of examples and documentation for these, while less so for platform agnostic .NET/C# WASM.  There are a multitude of options for interop between C# and JS types.  The rapid evolution of WASM implementations has led to documentation being inconsistent or outdated.  For example, some approaches documented as being specific to Blazor will actually work without Blazor, and vice versa.  This can result in each developer independently experimenting with approaches to discern the accuracy of documentation. 

This documentation focuses on platform agnostic approaches in the that are not dpenedent on the Blazor or Uno UI frameworks.  Uno.Bootstrap.Wasm is used as an example tool for compiling to a WASM compatible format, but otherwise code nd techniques largely focus on those that rely on .NET 7 JS interop capabilities, and would work in either framework as well as without them. 

The following represents approaches I've developed and believe to be effective.  Official documentation may differ, but I created this resource because I've found official documentation to sometimes be out of date, incomplete, or biased towards a platform specific approach.
